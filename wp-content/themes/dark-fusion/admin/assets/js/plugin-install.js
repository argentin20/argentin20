jQuery(document).ready(function($) {
    'use strict';
    var this_obj = dark_fusion_companion_install;
    ...
    $(document).on('click', '.spicethemes-plugin-activate', function(event) {
        ...
        var newURL2= newURL1 + "/admin.php?page=spice-blocks";
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'dark_fusion_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.spicethemes-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'dark_fusion_hide_customizer_notice';
            sdata.dark_fusion_plugin_slug = slug;
        }
        else {
            sdata.action = 'dark_fusion_hide_customizer_companion_notice';
        }
